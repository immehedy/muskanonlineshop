        ...
        {steps.map((stepInfo, index) => (
          <div key={stepInfo.num} className="flex items-center">
            <div className="flex flex-col items-center">
              ...
              <span className="mt-2 text-sm font-medium text-gray-700 dark:text-gray-300 tracking-wide">
                {stepInfo.label}
              </span>
            </div>
            {index < steps.length - 1 && (
              <div className="w-16 h-1 bg-gray-300 dark:bg-gray-600 mx-2 rounded-full" />
            )}
          </div>
        ))}
      </div>
