...
const Hero = ({hero} : any) => {
  ...
  const {title, subTitle, ctaText, ctaLink, backgroundImage} = hero

  console.log("imag", backgroundImage?.fields?.file);
  return (
    <div className="relative bg-accent overflow-hidden pb-10">
      <div className="container mx-auto relative z-10">

        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mt-4 place-items-center md:items-start text-center md:text-left">
          {/* Hero Text */}
          ...
          {title}
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              <Link href={`/${ctaLink}`} className="inline-flex items-center justify-center px-6 py-3 bg-[#247a95] text-white font-medium rounded-lg hover:bg-[#247a95]/90 transition-colors shadow-lg shadow-[#247a95]/20">
                {ctaText} <ChevronRight className="ml-2 w-5 h-5" />
              </Link>
              <Link href="/collections/new-arrivals" className="inline-flex items-center justify-center px-6 py-3 bg-accent-foreground/10 text-accent-foreground font-medium rounded-lg hover:bg-accent-foreground/20 transition-colors">
            ...
            <div className="grid grid-cols-3 gap-2 pt-10">
              <div className="flex flex-col items-center p-3 bg-background/80 backdrop-blur-sm rounded-lg border border-muted">
                ...
                <span className="text-xs text-center font-medium">Free Premium Shipping</span>
              </div>
              <div className="flex flex-col items-center p-3 bg-background/80 backdrop-blur-sm rounded-lg border border-muted">
                <RotateCcw className="w-6 h-6 text-[#247a95] mb-2" />
                <span className="text-xs text-center font-medium">30-Day Returns</span>
              </div>
              <div className="flex flex-col items-center p-3 bg-background/80 backdrop-blur-sm rounded-lg border border-muted">
                <Shield className="w-6 h-6 text-[#247a95] mb-2" />
                <span className="text-xs text-center font-medium">5-Year Warranty</span>
              </div>
            </div>
            
            <div className="pt-2">
              <p className="text-sm text-muted-foreground mt-2">
                Trusted by over <span className="font-medium text-foreground">2,000+</span> satisfied customers worldwide
              </p>
            </div>
          </div>
